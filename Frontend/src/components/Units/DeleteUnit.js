...
export default function DeleteUnit({deleteModel , setUpdatePage , updatePage , singleUnit}) {
    ...
    const deleteItem = async () => {
        ...
        try {
            const response = await fetch(`${GlobalApiState.DEV_BASE_LIVE}/api/unit/delete_unit/${singleUnit._id}`, {            method: 'DELETE'
          });
                                ...
                                <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                                    <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                        <p className="text-xl text-center text-gray-500">
                                            Are you sure you want to delete this Unit?</p>
                                    </div>
